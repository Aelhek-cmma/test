...
public class ServicesCommon extends BaseBO implements DefaultServicesCommon
{
	...
	try
	{
	    //set default timezone if exists as a property, otherwise machine timezone is used.
...
//	servicesCommonDAO.applyDbVersion();
	...
	synchronized(sqlSessionLock)
	{
	    ...
	    {
		log.error(e, "Error in Reading Property database.sqlsession.termination.enabled from PathRemoting.properties");
	    }
	}
	...
	synchronized(clusteredOperationLock)
	{
	    ...
	    try
	    {
		globalClusterEnabled = Boolean.parseBoolean(PathPropertyUtil.returnPathPropertyFromFile("PathRemoting",
