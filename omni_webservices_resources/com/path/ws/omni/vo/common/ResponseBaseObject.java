...
public class ResponseBaseObject extends RequestBaseObject implements Serializable
{
    ...
    /**
     * @author alishreif - add responseCode, responseDesc to distinguish between
     *         the case of error where errorCode will be not null and the case
     *         of success call where we need to return different codes for the
     *         success with different messages.
     * @return
     */
    ...
    /**
     * @author AliShreif
     * adding service response common section to handle the error message to be returned for the user
     */
   // nabil feghali : when trying to call the OmniEventBO from RMI we faced an error NotSerializableException in ServiceResponseVO because it is not serializable, after adding serilaizable it was fixed.
   // then we found that this class is linked from digital_banking_layer and should not be used here. 
   // private ServiceResponseVO serviceResponse;
     ...
     * To set Translated Message Code Description in outputNotification field by OmniServicesInterceptor.java 
     */ 
