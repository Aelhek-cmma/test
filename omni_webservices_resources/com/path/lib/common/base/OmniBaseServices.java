...
public class OmniBaseServices extends WebServiceBaseBO
{
    ...
    /**
     * @author AliShreif
     * this method is created to handle the translation of the exception thrown by the BOs 
     */
	...
	try{
	    ...
	    Integer errorCode = Integer.parseInt(StringUtil.nullEmptyToValue(
		    (((HashMap<String, Object>)hashOut.get(new String("serviceResponse"))).get(new String("statusCode"))),"-1"));

	    String errorDesc = StringUtil.nullEmptyToValue(
		    (((HashMap<String, Object>)hashOut.get(new String("serviceResponse"))).get(new String("statusDesc"))),"Contact Administrator");
	    ...
	    } catch (SecurityException e) { pathlog.error(e, e.getMessage()); 
	    }
	    catch (NoSuchMethodException e) { pathlog.error(e, e.getMessage());
	    }
	    ...
	    } catch (IllegalArgumentException e) { pathlog.error(e, e.getMessage());
	    }
	    catch (IllegalAccessException e) {pathlog.error(e, e.getMessage());
	    }
	    catch (InvocationTargetException e) { pathlog.error(e, e.getMessage());
	    }
